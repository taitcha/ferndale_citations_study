...
def runVeil(citations, filename):
    ...
    citationsV["CITATION_DT"] = citationsV["CITATION_DT"].apply(addTimeZone)

    # Make Race binary (0 = W, 1 = B)
    citationsV["OFF_RACE_CD"] = citationsV.apply(lambda row: 0 if row["OFF_RACE_CD"] == "W" else 1, axis=1)
    ...
    citationsV["DAWN"] = citationsV.apply(lambda row: city.sun(row['CITATION_DT'])['dawn'], axis=1)
    citationsV["SUNRISE"] = citationsV.apply(lambda row: city.sun(row['CITATION_DT'])['sunrise'], axis=1)
    citationsV["DUSK"] = citationsV.apply(lambda row: city.sun(row['CITATION_DT'])['dusk'], axis=1)
    citationsV["SUNSET"] = citationsV.apply(lambda row: city.sun(row['CITATION_DT'])['sunset'], axis=1)
    citationsV["WEEKDAY"] = citationsV.apply(lambda row: row['CITATION_DT'].weekday(), axis=1)
    citationsV["TIME"] = citationsV.apply(lambda row: row['CITATION_DT'].time(), axis=1)

    # Mark if citation is during daytime
    def checkDaylight(row):
        if (row["CITATION_DT"].time() > row["SUNRISE"].time()) and (row["CITATION_DT"].time() < row["SUNSET"].time()):
            return True
        else:
            return False
    ...
    def checkIntertwilight(row):
        if (row["CITATION_DT"].time() >= earlySunset) and (row["CITATION_DT"].time() <= lateSunset):
    ...
    # Fit GLM regression
    formula = "OFF_RACE_CD ~ WEEKDAY + DAY_STRICT_FLAG"
    ...
    citationsInt["SAMEDATE"] = citationsInt.apply(lambda row: datetime.datetime.combine(date,row['CITATION_DT'].time()), axis=1)
    timeindexDay = citationsInt[citationsInt["DAY_STRICT_FLAG"]==True]
    timeindexDay = timeindexDay.filter(["SAMEDATE","OFF_RACE_CD","DAY_STRICT_FLAG"])
    ...
    timeindexNight = timeindexNight.filter(["SAMEDATE","OFF_RACE_CD","DAY_STRICT_FLAG"])
